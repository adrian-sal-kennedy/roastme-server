...
class FollowController < ApplicationController
  ...
  def follow
    skip = false
    current_user.following.followings_users.where(user_id: params[:id]).each do |_user|
      skip = true
    end
    if skip == false
  ...
  def following?
    following = false
    current_user.following.followings_users.where(user_id: params[:id]).each do |_follow|
      following = true
    end
